...
export class GagnantUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.jeuService
      ...
      .subscribe(
        (res: IJeu[]) => {
          if (!this.editForm.get('jeu').value || !this.editForm.get('jeu').value.id) {
            this.jeus = res;
          } else {
            this.jeuService
              .find(this.editForm.get('jeu').value.id)
              .pipe(
                filter((subResMayBeOk: HttpResponse<IJeu>) => subResMayBeOk.ok),
                map((subResponse: HttpResponse<IJeu>) => subResponse.body)
              )
              .subscribe((subRes: IJeu) => (this.jeus = [subRes].concat(res)), (subRes: HttpErrorResponse) => this.onError(subRes.message));
          }
        },
